...
# This script patches the RCTThirdPartyComponentsProvider.mm file to prevent crashes
# when the new architecture is disabled but codegen files are still generated.
# It filters out nil classes to prevent crashes when creating the NSDictionary.

FILE_PATH="ios/build/generated/ios/RCTThirdPartyComponentsProvider.mm"

if [ ! -f "$FILE_PATH" ]; then
  echo "RCTThirdPartyComponentsProvider.mm not found, skipping patch"
  exit 0
fi

echo "Patching RCTThirdPartyComponentsProvider.mm to handle nil classes..."
...
+ (NSDictionary<NSString *, Class<RCTComponentViewProtocol>> *)thirdPartyFabricComponents
{
  ...
  dispatch_once(&nativeComponentsToken, ^{
    ...
    // Filter out nil classes to prevent crashes when new architecture is disabled
    NSDictionary *potentialComponents = @{
    ...
    // Only add non-nil classes to the components dictionary
    for (NSString *key in potentialComponents) {
      Class cls = potentialComponents[key];
      if (cls != nil) {
        components[key] = cls;
      }
    }
...
echo "✅ Successfully patched RCTThirdPartyComponentsProvider.mm"
